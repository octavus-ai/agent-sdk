...
| `display`     | No       | How to show in UI: `title`, `stream`, or `hidden`. Set it on every tool - an unset `display` falls back to the deprecated `description` mode (see Display Modes) |
| `title`       | No       | User-facing UI label for `title` and `stream` modes. Set it on every `stream` tool - without one the label falls back to the `description`, which is deprecated  |
| `parameters`  | No       | Input parameters the tool accepts                                                                                                                                |

### Display Modes

Controls what the client sees about tool execution. There are three modes. Set `display` on every tool: a tool with no `display` falls back to the deprecated `description` mode and validation warns. (MCP servers, skills, and operation-style blocks default to `title`.)

| Mode     | Behavior                                                                                                                                                                                                                                                                                                                                                             |
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `title`  | **Recommended for tools; the default everywhere else.** A label-only indicator: the tool's `title` (falling back to the tool name) plus the tool name that drives the icon. Arguments and result are hidden in the UI; the `description` is still sent to the LLM. Use for tools that should appear as a clean, labeled step without exposing inputs/outputs.        |
| `stream` | Full visibility. Arguments stream progressively as the LLM generates them, and the result is shown after execution (and preserved after refresh). Labeled with the `title`; a `stream` tool without one falls back to the `description` for backward compatibility and validation warns, so set a `title`. Use when the user benefits from seeing arguments/results. |
...
Render the label as `title ?? displayName ?? toolName`: `title` carries the authored label in `title` mode, `displayName` carries the label for every other visible mode (the authored `title` for `stream`, else its description fallback; the `description` for the deprecated mode), and `toolName` (which also drives the icon) is the fallback your UI formats. Write the `description` for the model and the `title` for the user.

> **Deprecated modes.** `name` (shows the tool name only) and `description` (shows the `description` as the label) still work for backward compatibility, but protocol validation emits a deprecation warning - whether set explicitly or reached by leaving a tool's `display` unset. Prefer `title` (a clean label plus the name) or `stream` (full visibility).
