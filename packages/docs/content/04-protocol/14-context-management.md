...
- The same automatic behavior covers **images**. Long computer-use or document sessions accumulate screenshots and image reads, and providers cap how many images - and how large a request body - a single request may carry. For a provider whose per-request image limit is known, the runtime stays under it **proactively**, dropping the oldest images while keeping the most recent, before any rejection - so the common path never hits a hard failure. It prunes in blocks rather than one image per step, so the prompt cache keeps hitting between prunes. If a provider rejects anyway (too many images, an image that is too large - the sort of limit [`maxImageDimension`](/docs/protocol/agent-config#image-delivery-limits) is designed to pre-empt - or an oversized request body), the agent adapts the outgoing images and retries, dropping the oldest to satisfy a count or body-size limit and downscaling an over-large image, then remembers what it learned so later steps comply up front. It never re-sends a payload that just failed, and a deterministic image or size rejection is reported as such - never as a "temporary error, please retry". As with token overflow, this is only active when `contextManagement` is declared; without it, the rejection surfaces as a clear, well-classified failure. Every adaptation is recorded as an `image-adapted` entry in the execution logs, and the original full-resolution images are always preserved in stored history and downloads.
